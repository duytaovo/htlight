const handleData2 = (data: any, filter: any) => {
  let gia: any = 0;
  let price: any = 0;
  ...
  const dataAfter = data?.filter((e: any) => {
    if (e.parameter) {
      ...
      for (const [key, value] of Object?.entries(e?.parameter)) {
        // Lọc qua từng phần tử trong filter
        let duplicate: any = [];
        let flag = false;
        const checkTitle = filter.some((element: any) => {
          ...
          if (duplicate.includes(keyM[0])) {
            ...
            duplicate.push(keyM[0]);
            if (keyM[0] === key) {
              if (typeof valueM[0] === "string" && valueM[0] === value)
                flag = true;
              else {
                // if (value.includes(valueM[0])) flag = true;
              }
            }
          }
        });
